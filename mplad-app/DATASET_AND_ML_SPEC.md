DATASET & ML TRAINING SPEC — PRAHARI (v2, cleaned)
SINGLE SOURCE OF TRUTH for the dataset. STEP_01 implements §1 verbatim.
If this file is not committed in the repo, the agent will improvise —
that is how the ₹1,226 cr bug happened. Never let the repo lose it.

§0 INVENTORY
File	Feeds
works.csv, mps/districts/agencies/villages.csv	all detectors, API, frontend
fund_flows.csv	D4 utilization
work_specs.csv	Step 09 BoQ synthesis
fraud_labels.csv (fraud/inefficiency/violation/innocent)	eval harness
data/norms/norm_items.csv, price_indices.csv	Step 09 cost engine

§1 GENERATOR — seed/generate_dataset.py (implement exactly)
```python
#!/usr/bin/env python3
...
NXT = {"2019-20":"2021-22","2021-22":"2022-23","2022-23":"2023-24","2023-24":"2023-24"}
...
for _ in range(20):                                                     # P6 copy-paste
    ...
    rs = mps[(mps.house=="RS") & (mps.state==st)].iloc[0]
...
for _ in range(100):                                                    # P4 stalled
    ...
    unit, qlo, qhi, plo, phi = CATS[cat][:5]
    dcode = districts.id.iloc[int(rng.integers(12))]
    s = REF - pd.Timedelta(days=int(rng.uniform(420,900)))
    wid = add_work(mps.iloc[int(rng.integers(20))], D(dcode), cat,
...
for k in range(25):                                                     # P8 prohibited
    ...
    s = fy_start("2023-24") + pd.Timedelta(days=30*k)
...
§2 NORM ITEMS STARTER — data/norms/norm_items.csv
...
