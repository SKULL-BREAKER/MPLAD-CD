...
async function handleUploadEvidence(formData) {
  ...
  if (!work_id) return;
  try {
    await appendEvidence(work_id, media_type, lat, lng, authority);
  } catch (e) {
                ...
                {w.evidence.length > 0 && (
                  <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '10px 14px', marginBottom: '16px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                      {w.evidence.map(e => (
                        <div key={e.id} style={{ display: 'flex', gap: '12px', fontSize: '0.78rem', color: 'var(--text-muted)', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', paddingTop: '12px', borderTop: '1px solid var(--border-color)' }}>
                  ...
                  <form action={handleUploadEvidence} style={{ display: 'flex', gap: '8px', flex: 1, alignItems: 'center', flexWrap: 'wrap' }}>
                    ...
                    <input name="latitude" type="number" step="0.00001" min="-90" max="90"
                      ...
                      style={{ width: '120px', margin: 0, padding: '8px 10px', fontSize: '0.8rem' }} />
