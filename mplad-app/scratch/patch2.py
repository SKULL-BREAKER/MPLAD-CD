...
for _ in range(60):
    for attempt in range(3):
        ...
        fy_str = NXT[w["fy"]]
        est_amt = w["sanctioned_amount"] * 1.0
    ...
    cw = works[-1]
    old_amt = cw["sanctioned_amount"]
    
    new_amt = round(w["sanctioned_amount"]*rng.uniform(.90,1.10), 0)
    if not fits(w["mp_id"], fy_str, new_amt - old_amt):
        avail = max(0, 1.15 * FY_ENT[fy_str] - cell_sums[(w["mp_id"], fy_str)])
        new_amt = old_amt + avail
        
    cw["sanctioned_amount"] = new_amt
    cw["expenditure"] = round(cw["sanctioned_amount"]*rng.uniform(.85,1.0), 0)
    cell_sums[(cw["mp_id"], cw["fy"])] += (cw["sanctioned_amount"] - old_amt)
...
p6_new = '''for _ in range(20):                                                     # P6 copy-paste
    ...
    for dcode in rng.choice(others, size=2, replace=False):
        ...
        new_amt = round(w["sanctioned_amount"]*rng.uniform(.95,1.05), 0)
        if not fits(rs["id"], w["fy"], new_amt - old_amt):
            avail = max(0, 1.15 * FY_ENT[w["fy"]] - cell_sums[(rs["id"], w["fy"])])
            new_amt = old_amt + avail
            
        cw["sanctioned_amount"] = new_amt
        cw["expenditure"] = round(cw["sanctioned_amount"]*rng.uniform(.85,1.0), 0)
        cell_sums[(rs["id"], w["fy"])] += (cw["sanctioned_amount"] - old_amt)
        label(wid, "P6", "fraud")
    label(w["id"], "P6", "fraud")'''
...
p4_new = '''for _ in range(100):                                                    # P4 stalled
    ...
    if not fits(mp["id"], fy_str, est_amt):
        avail = max(0, 1.15 * FY_ENT.get(fy_str, 5.0e7) - cell_sums[(mp["id"], fy_str)])
        if est_amt > 0: perunit *= avail / est_amt
