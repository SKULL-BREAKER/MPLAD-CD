...
# Honest loop
honest_old = """            while spent < budget and guard < 500:
                guard += 1
                cat = CAT_NAMES[int(rng.choice(len(CAT_NAMES), p=CAT_W))]
                unit, qlo, qhi, plo, phi = CATS[cat][:5]
...
honest_new = """            while spent < budget and guard < 500:
                guard += 1
                cat = CAT_NAMES[int(rng.choice(len(CAT_NAMES), p=CAT_W))]
                unit, qlo, qhi, plo, phi = CATS[cat][:5]
                qty = (round(float(rng.uniform(qlo,qhi)),1) if unit=="km"
                ...
                est_amt = qty * float(rng.uniform(plo,phi)) * cat_ratio(cat, sanction) * float(d.terrain)
                if not fits(mp["id"], fy, est_amt):
                    if guard > 450: break
                    continue
...
code = code.replace(honest_old, honest_new)
